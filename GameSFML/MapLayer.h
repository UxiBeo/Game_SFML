...
public:
        ...
        : m_animTiles(animTiles)
    {
        ...
        layerOffset.x = (float)offset.x;
        layerOffset.x = (float)offset.y;
        ...
        for (const auto& ts : tilesets)
        {
            ...
            m_chunkArrays.emplace_back(Codex<TextureResource>::Retrieve(entt::hashed_string{ ts->getImagePath().c_str() }).data, *ts);
        }
                ...
                for (auto x = xPos; x < xPos + chunkTileCount.x; ++x)
                {
                        ...
                        && m_chunkTileIDs[idx].ID <= ca.m_lastGID)
                    {
                        if (registerAnimation && m_animTiles.find(m_chunkTileIDs[idx].ID) != m_animTiles.end())
                        {
                            ...
                            as.tileCords = sf::Vector2u((unsigned int)x, (unsigned int)y);
                            m_activeAnimations.push_back(as);
                        }
                        const auto caf = sf::Vector2f((float)ca.tileSetSize.x, (float)ca.tileSetSize.y);
                        sf::Vector2f tileOffset((float)x * mapTileSize.x, (float)y * (float)mapTileSize.y + (float)mapTileSize.y - caf.y);

                        auto idIndex = m_chunkTileIDs[idx].ID - ca.m_firstGID;
                        sf::Vector2f tileIndex(float(idIndex % ca.tsTileCount.x), float(idIndex / ca.tsTileCount.x));
                        tileIndex.x *= caf.x;
                        tileIndex.y *= caf.y;
                        ChunkArray::Tile tile =
                        {
                            sf::Vertex(tileOffset - getPosition(), m_chunkColors[idx], tileIndex),
                            sf::Vertex(tileOffset - getPosition() + sf::Vector2f(caf.x, 0.f), m_chunkColors[idx], tileIndex + sf::Vector2f(caf.x, 0.f)),
                            sf::Vertex(tileOffset - getPosition() + caf, m_chunkColors[idx], tileIndex + caf),
                            sf::Vertex(tileOffset - getPosition() + sf::Vector2f(0.f, caf.y), m_chunkColors[idx], tileIndex + sf::Vector2f(0.f, caf.y))
                        };
    ...
    {
        return x + int((float)y * chunkTileCount.x);
    }
...
private:
    ...
    std::vector<ChunkArray> m_chunkArrays;
    void draw(sf::RenderTarget& rt, sf::RenderStates states) const override
    {
    ...
    void createChunks(const tmx::Map& map, const tmx::TileLayer& layer)
    {
        ...
        for (auto y = 0u; y < m_chunkCount.y; ++y)
        {
            ...
            for (auto x = 0u; x < m_chunkCount.x; ++x)
            {
                ...
                m_chunks.emplace_back(layer, usedTileSets,sf::Vector2f(x * m_chunkSize.x, y * m_chunkSize.y), 
                    tileCount, tileSize, map.getTileCount().x, map.getAnimatedTiles());
            }
        }
    }
    ...
    void updateVisibility(const sf::View& view) const
    {
        ...
        for (auto y = posY; y < posY2; ++y)
        {
            for (auto x = posX; x < posX2; ++x)
            {
                ...
                {
                    visible.push_back(const_cast<Chunk*>(&m_chunks[idx]));
                }
            }
        }
